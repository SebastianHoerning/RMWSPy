#-------------------------------------------------------------------------------
# Purpose:     Precipitation Simulation combining rain gauges and
#              commercial microwave link data
#
# Author:      Dr.-Ing. Sebastian Hoerning
#
# Created:     15.01.2020, Centre for Natural Gas, EAIT, 
...
try:
	from random_mixing_whittaker_shannon import RMWSCondSim
except:
...
def main():
	...
	for tstep in range(start_time_idx, end_time_idx):
		...
		# non-linear additional variables
		nl_variables = {}
		nl_variables['p0'] = p0
		nl_variables['cdf'] = cdf
		nl_variables['invcdf'] = invcdf

		# initialize Random Mixing Whittaker-Shannon
		CS = RMWSCondSim(domainsize = (ysize, xsize),
						 covmod = cmod,
						 nFields = nfields,
						 cp = cp,
						 cv = cv,
