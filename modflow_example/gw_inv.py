...
try:
	...
	# add path to flopy!
	fpth = os.path.abspath(os.path.join(r''))
	sys.path.append(fpth)
	import flopy
...
def main():
	...
	nl_variables = {}
	nl_variables['dist_par'] = dist_par
	nl_variables['laytyp'] = laytyp
	nl_variables['kv'] = kv
	nl_variables['MF'] = MF
	nl_variables['flopy'] = flopy
	nl_variables['headfile'] = headfile

	# initialize Random Mixing Whittaker-Shannon
	CS = RMWSCondSim(domainsize = (nrow, ncol),
					 covmod = cmod,
					 nFields = nfields,
					 cp = obswell,
					 cv = cv,
